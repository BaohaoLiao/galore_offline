...
class AdamWColumnImportance(Optimizer):
    """
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "sample_ratio" in group:
                    ...
                    # avg_abs_grad.mul_(state["step"] - 1).add_(grad.abs().mean(dim=0)).div_(state["step"])
                    ...
                    if state["step"] < sample_start:
                        ...
                        # Update average absolute gradient for each row
                        avg_abs_grad.mul_(state["step"] - sample_start).add_(grad.abs().mean(dim=0)).div_(state["step"] - sample_start + 1)
