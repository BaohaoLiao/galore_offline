...
def main(args):
    ...
    if args.optimizer.lower() == "adamw_lora" or args.optimizer.lower() == "adamw_lora_rectified" or args.optimizer.lower() == "adamw_lora_update":
        ...
        for module_name, param in model.named_parameters():
            if 'lora_' not in module_name and not any(target_key in module_name for target_key in target_modules_list):
        ...
        for module_name, module in model.named_modules():
            ...
            lora_params_names.append(module_name)

        id_lora_params = [id(p) for p in lora_params]
        regular_params = [p for p in model.parameters() if id(p) not in id_lora_params]
        param_groups = [
            {"params": regular_params},
            {"params": lora_params, "rank": args.rank, "lora_target_keys": lora_target_keys, "glore_params_names": lora_params_names, 'scale': args.galore_scale, 'update_proj_gap': args.update_proj_gap},
        ]
    ...
    if args.optimizer.lower() == "adamw_lora":
        ...
        optimizer = transformers.optimization.AdamW(trainable_params, lr=args.lr, weight_decay=args.weight_decay)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if update_step % args.eval_every == 0:
            ...
            if global_rank == 0:
                ...
                """
                logger.info(f"\n{update_step}/{global_step} step, {evaluated_on_tokens} tokens\t|| eval loss: {total_loss:.3f}, eval ppl: {math.exp(total_loss):.2f}")
        ...
        if global_rank == 0:
            ...
            """
            logger.info(f"\n{update_step}/{global_step} step, {tokens_seen} tokens\t|| train loss: {loss.item():.3f}, lr: {lr:.6f}, " +
